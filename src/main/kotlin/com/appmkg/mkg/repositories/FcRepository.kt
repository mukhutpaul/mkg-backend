...
interface StagiaireRepository : JpaRepository<Stagiaire,Int> {
    ...
    @Query("select count(s.id) as nombre,f.title as titre from Stagiaire as s,Formation as f where s.formation_id = f.id group by s.formation_id,f.title", nativeQuery = true)
